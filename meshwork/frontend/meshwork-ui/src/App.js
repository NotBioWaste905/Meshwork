...
const sampleTasks = [
  {
    ...
    description: 'Create initial project plan and define requirements for the task management system',
    status: 2, // DONE
  ...
  },
  {
    ...
    description: 'Create React components for task visualization using ReactFlow',
    status: 1, // IN_PROGRESS
    users: ['sarah.connor'],
  ...
  },
  {
    ...
    description: 'Design and implement database schema for tasks and dependencies',
    status: 2, // DONE
    users: ['david.brown'],
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(fetchedTasks) && fetchedTasks.length > 0) {
          ...
          console.log('Backend available but no tasks found, using sample data');
        }
      } catch (backendError) {
        console.warn('Backend not available, using sample data:', backendError.message);
